...
export default function ChatListScreen({ navigation, route }) {
  ...
  const fetchFriends = async () => {
    try {
      let response = await fetch('https://message-app-qj28.onrender.com/friends', {
        method: 'GET',
        headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      let response = await fetch(`https://message-app-qj28.onrender.com/search_users?username=${searchQuery}`, {
  ...
  const handleAddFriend = async (friendId) => {
    try {
      const res = await fetch('https://message-app-qj28.onrender.com/users/add-friend', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': user.user_id,
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.searchItem}>
                  ...
                  <TouchableOpacity onPress={() => handleAddFriend(item._id ? item._id : item.id)} style={styles.addButton}>
