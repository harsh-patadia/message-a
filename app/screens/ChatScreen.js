...
export default function ChatScreen({ route }) {
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(`https://message-app-qj28.onrender.com/messages/${friend.id ? friend.id : friend._id}`, {
        method: 'GET',
        headers: {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('https://message-app-qj28.onrender.com/messages', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json', 
          'Authorization': user.user_id 
