...
export default function CreateGroupScreen({ navigation, route }) {
  const { user, onGroupCreated } = route.params;
  ...
  const handleSearch = async (text) => {
    ...
    try {
      const res = await fetch(`https://message-app-qj28.onrender.com/search_users?username=${text}`, {
        headers: {
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      const res = await fetch('https://message-app-qj28.onrender.com/groups/create', {
        method: 'POST',
        headers: {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={[
        styles.userItem,
        selectedUsers.includes(item._id ? item._id : item.id) && styles.userSelected,
      ]}
      onPress={() => toggleUserSelect(item._id ? item._id : item.id)}
    >
